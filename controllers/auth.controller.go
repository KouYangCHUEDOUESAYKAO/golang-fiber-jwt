...
func SignUpUser(c *fiber.Ctx) error {
	...
	if result.Error != nil && strings.Contains(result.Error.Error(), "duplicate key value violates unique") {
		return c.Status(fiber.StatusConflict).JSON(fiber.Map{"status": "error", "message": "Something bad happened"})
	}

	return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"status": "success", "data": fiber.Map{"user": models.FilterUserRecord(&newUser)}})
}
...
func SignInUser(c *fiber.Ctx) error {
	...
	return c.Status(fiber.StatusOK).JSON(fiber.Map{"status": "Login Success", "token": tokenString})
}
...
func LogoutUser(c *fiber.Ctx) error {
	...
	return c.Status(fiber.StatusOK).JSON(fiber.Map{"status": "Logout Success"})
}
